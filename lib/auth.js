...
/** Тухайн хэрэглэгчийн БҮХ session-ийг устгах.
    Нууц үг солих/сэргээх үед дуудна — хуучин (болзошгүй халдагчийн)
    session-ууд хүчингүй болно. */
function destroyUserSessions(userId) {
  const arr = db.table("sessions");
  let changed = false;
  for (let i = arr.length - 1; i >= 0; i--) {
    if (arr[i].userId === userId) { arr.splice(i, 1); changed = true; }
  }
  if (changed) db.save();
}
