...
async function handleApi(req, res, pathname, query) {
  ...
  try {
    ...
    // ---------- ADMIN: backup export ----------
    if (seg[0] === "admin" && seg[1] === "export" && method === "GET") return exportDb(req, res);

    // ---------- USERS (админ: хэрэглэгчдийн бүртгэл) ----------
    if (seg[0] === "users" && !seg[1] && method === "GET") return listUsers(req, res);

    // ---------- REVIEWS (сэтгэгдэл) ----------
    if (seg[0] === "reviews") {
      if (!seg[1]) {
        if (method === "GET") return listReviews(req, res);
        if (method === "POST") return createReview(req, res);
      } else if (method === "DELETE") {
        return deleteReview(req, res, seg[1]);
      }
    }
...
function validateProductPayload(body, partial) {
  ...
  if (has("image")) out.image = U.str(body.image, 500);
  if (!partial || has("concentration")) {
    const CONC = ["", "EDP", "EDT", "EDC", "Parfum", "Extrait"];
    out.concentration = CONC.indexOf(body.concentration) > -1 ? body.concentration : "";
  }
  if (!partial || has("discount")) {
    const d = Math.round(Number(body.discount) || 0);
    out.discount = d >= 0 && d <= 90 ? d : 0;
  }
  if (!partial || has("stock")) {
    // Хоосон = хязгааргүй нөөц (декантад тохирно). Тоо = үлдэгдэл ширхэг.
    if (body.stock === "" || body.stock === null || body.stock === undefined) {
      out.stock = "";
    } else {
      const s = Math.round(Number(body.stock));
      out.stock = isFinite(s) && s >= 0 ? s : "";
    }
  }
  return out;
}
...
async function createProduct(req, res) {
  ...
  data.stock = data.stock === undefined ? "" : data.stock;
...
async function createOrder(req, res) {
  ...
  for (const it of rawItems) {
    ...
    let qty = Math.max(1, Math.min(99, parseInt(it.qty, 10) || 1));
    if (!product || product.active === false) continue;
    if (SETTINGS.sizes.indexOf(ml) === -1) continue;
    // Нөөц хяналт: тоо тавьсан бол шалгана (хоосон = хязгааргүй)
    const tracked = product.stock !== "" && product.stock !== null && product.stock !== undefined;
    if (tracked) {
      const avail = Math.max(0, Math.round(Number(product.stock) || 0));
      if (avail <= 0) continue;        // дууссан — алгасна
      if (qty > avail) qty = avail;    // боломжит хэмжээнд багасгана
    }
    const base = Number(product.prices[ml]) || 0;
    const disc = Math.round(Number(product.discount) || 0);
    const unitPrice = (disc > 0 && disc < 100) ? Math.round(base * (100 - disc) / 100) : base;
  ...
  // Нөөцөөс хасах (зөвхөн тоо тавьсан бараанаас)
  items.forEach(function (it) {
    const p = db.find("products", it.productId);
    if (p && p.stock !== "" && p.stock !== null && p.stock !== undefined) {
      const left = Math.max(0, Math.round(Number(p.stock) || 0) - it.qty);
      db.update("products", p.id, { stock: left });
    }
  });
  ...
  // Захиалагчид (үйлчлүүлэгчид) баталгаажуулах и-мэйл (алдаа гарвал захиалга хэвээр)
  try {
    const cm = mail.customerOrderEmail(order);
    await mail.sendMail({ to: order.userEmail, subject: cm.subject, html: cm.html });
  } catch (e) { console.error("Захиалагчийн и-мэйл алдаа:", e.message); }
...
async function updateOrder(req, res, id) {
  ...
  // Төлөв өөрчлөгдвөл захиалагчид и-мэйл мэдэгдэл (алдаа гарвал шинэчлэл хэвээр)
  if (patch.status && patch.status !== oldStatus && updated.userEmail) {
    try {
      const sm = mail.orderStatusEmail(updated, patch.status);
      if (sm) await mail.sendMail({ to: updated.userEmail, subject: sm.subject, html: sm.html });
    } catch (e) { console.error("Төлөв мэдэгдэл алдаа:", e.message); }
  }
...
/* ================================================================== */
/*  ADMIN: backup export (бүх өгөгдлийг татаж авах)                    */
/* ================================================================== */
function exportDb(req, res) {
  ...
  res.writeHead(200, {
    ...
    "Content-Disposition": 'attachment; filename="decante-backup-' + stamp + '.json"',
...
/* ================================================================== */
/*  USERS (админ: хэрэглэгчдийн бүртгэлийг хянах)                      */
/* ================================================================== */
function listUsers(req, res) {
  const ctx = requireAdmin(req, res);
  if (!ctx) return;
  const orders = db.table("orders");
  const users = db.filter("users", (u) => u.role !== "admin").map((u) => {
    const uOrders = orders.filter((o) => o.userId === u.id);
    const spent = uOrders
      .filter((o) => o.status !== "cancelled")
      .reduce((s, o) => s + (Number(o.total) || 0), 0);
    const last = uOrders.reduce((m, o) => Math.max(m, o.createdAt || 0), 0);
    return {
      id: u.id,
      name: u.name,
      email: u.email,
  ...
  return U.sendJson(res, 200, { users: users });
}

/* ================================================================== */
/*  REVIEWS (хэрэглэгчийн сэтгэгдэл — нүүрэнд харагдана)                */
/* ================================================================== */
function listReviews(req, res) {
  const list = db.table("reviews").slice().sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
  return U.sendJson(res, 200, { reviews: list });
}
...
async function createReview(req, res) {
  ...
  if (!ctx) return;
  const body = await U.readJsonBody(req);
  const text = U.str(body.text, 400);
  const author = U.str(body.author, 80);
  if (!text) return U.sendError(res, 400, "Сэтгэгдлийн текст шаардлагатай.");
  const review = db.insert("reviews", {
    id: U.genId("review"),
    text: text,
    author: author || "Үйлчлүүлэгч",
    createdAt: Date.now(),
  });
  return U.sendJson(res, 201, { review: review });
}

function deleteReview(req, res, id) {
  const ctx = requireAdmin(req, res);
  if (!ctx) return;
  const ok = db.remove("reviews", id);
  if (!ok) return U.sendError(res, 404, "Сэтгэгдэл олдсонгүй.");
  return U.sendJson(res, 200, { ok: true });
}
